...
Real robot flow:
  ...
  6. Move right arm to READY.
  7. Optional Dex3 OPEN while holding READY.
  8. Move right arm READY -> IK target.
  9. Optional Dex3 close while holding IK target, stopping early on pressure delta.
 10. Release arm_sdk weight 1 -> 0.
...
def solve_ik_for_target(target_forward: float, target_left: float, target_up: float) -> Optional[Dict[str, float]]:
...
class Controller:
    ...
    def _tick(self):
        ...
        if self.stage == "weight_up":
            ...
            self.low_cmd.motor_cmd[WEIGHT_SLOT].q = 1.0
            q = self._blend(self.clearance_q, self.ready_q, t_norm)
            self._write_arms(q)
            self._publish()
            if t_norm >= 1.0:
            ...
            last_q = self.ik_cmd_q if (self.ik_cmd_q is not None and not self.ready_only) else self.ready_q
...
def parse_args():
    ...
    p.add_argument("--ws-min-up", type=float, default=0.05)
...
def main():
    ...
    ik_result = solve_ik_for_target(args.target_forward, args.target_left, args.target_up)
    ...
    print(f"\n{'joint':<22} {'~rest':>10} {'ready':>10} {'IK raw':>10} {'IK cmd':>10} {'kp':>7}")
    for i, name in enumerate(RIGHT_ARM_NAMES):
        print(f"{name:<22} {rest_approx[i]:+10.4f} {ready_q[i]:+10.4f} {ik_q[i]:+10.4f} {ik_cmd_q[i]:+10.4f} {per_joint_kp[i]:7.1f}")
    print(f"{'=' * 78}")
