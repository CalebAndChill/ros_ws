...
class Yolo3DSpeaker(Node):
    ...
    def say(self, text: str) -> bool:
        ...
        # Even in log-only mode, keep the same pacing
    ...
    def tick(self):
        ...
        # Wait until current sentence is likely finished
        if now < self.speaking_until:
            return

        # If we already have queued lines, speak the next one first
        if self.pending_lines:
            line = self.pending_lines.pop(0)
            self.say(line)
            return

        # Only build a fresh batch on your chosen cadence
